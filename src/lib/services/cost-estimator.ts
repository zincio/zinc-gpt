export interface CostEstimate {
  ...
  total: number         // cents
  taxRate: number       // decimal (e.g., 0.0725)
  state: string         // 2-letter state code
}

// Zip code prefix (first 3 digits) to state mapping
// Source: USPS zip code ranges by state
...
// State sales tax rates (base rates as of 2024)
// Note: These are BASE state rates. Local taxes can add 1-4% more.
 ...
 * Formula: clamp(price * 0.10, 599, 2999) in cents
 * This is a heuristic estimate
 */
function estimateShipping(priceCents: number): number {
  const minShipping = 599   // $5.99
  const maxShipping = 2999  // $29.99
  const shippingRate = 0.10 // 10% of product price

  const estimated = Math.round(priceCents * shippingRate)
  return Math.max(minShipping, Math.min(maxShipping, estimated))
}

/**
 * Estimate total costs (subtotal, tax, shipping) for a product
 * @param productPriceCents - Product price in cents
 * @param zipCode - 5-digit US zip code
 * @returns Cost breakdown or null if invalid zip code
 */
export function estimateCosts(
  ...
  const tax = Math.round(productPriceCents * taxRate)

  // Estimate shipping
  const shipping = estimateShipping(productPriceCents)

  // Calculate total
  const total = productPriceCents + tax + shipping

  return {
    subtotal: productPriceCents,
    tax,
    shipping,
    total,
    taxRate,
